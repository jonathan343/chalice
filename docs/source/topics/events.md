...
<a id="scheduled-events"></a>
## Scheduled Events

Chalice has support for scheduled events. This feature allows you to
...
The `Chalice.schedule` method accepts either a string or an
instance of `Rate` or `Cron`. For example:
...
See the [CloudWatch Event pattern docs](https://amzn.to/2OlqZso)
...
This is done with the `Chalice.on_s3_event` decorator.
...
`event` parameter, which is of type `S3Event`.
...
`Chalice.on_sqs_message` decorator and provide the name of the SQS queue
...
[AWS CLI](https://docs.aws.amazon.com/cli/latest/reference/sqs/get-queue-attributes.html),
...
See [lambda config](configfile.md#lambda-config) for a list of all supported lambda configuration
...
invoked. The function argument is an `SQSEvent` object, and each
`record` in the example above is of type `SQSRecord`. Lambda takes
care of automatically scaling your function as needed. See [Understanding
Scaling Behavior][understanding scaling behavior] for more information on how Lambda scaling works.
...
`SQSEvent`. You don't need to manually call `sqs.delete_message()`
...
<a id="kinesis-events"></a>
## Kinesis Events

You can configure a Lambda function to be invoked whenever messages are
published to an Amazon Kinesis data stream. To configure this, use the
`Chalice.on_kinesis_record` decorator and provide the name of the
Kinesis stream.

The `KinesisEvent` that is passed in as the `event` argument
...
<a id="dynamodb-events"></a>
## DynamoDB Events

You can configure a Lambda function to be invoked whenever messages are
published to an Amazon DynamoDB stream. To configure this, use the
`Chalice.on_dynamodb_record` decorator and provide the name of the
DynamoDB stream ARN.

!!! note
    Other event handlers such as `Chalice.on_kinesis_record`,
    `Chalice.on_sqs_message`, and `Chalice.on_sns_message`
...
The `DynamoDBEvent` that is passed in as the `event` argument
to the event handler is also iterable. This allows you to iterate over
...
[aws documentation]: https://docs.aws.amazon.com/lambda/latest/dg/with-sqs.html
