...
object with an appropriately set status code (see [view error handling](views.md#view-error-handling)).
...
You can also use the `Chalice.register_middleware` method, which
has the same behavior as `Chalice.middleware` except you provide
...
You can also use the `ConvertToMiddleware` class to convert an
...
Lambda wrappers. See [powertools example](middleware.md#powertools-example) for a more complete
example.
...
<a id="powertools-example"></a>
### Integrating with AWS Lambda Powertools

[AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/python/latest/) is a suite of
...
and [Tracer](https://docs.powertools.aws.dev/lambda/python/latest/core/tracer/)
