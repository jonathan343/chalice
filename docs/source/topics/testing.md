...
Chalice provides a [test client](../api.md#testing-api) in `chalice.test` that
...
<a id="testing-lambda-functions"></a>
## Lambda Functions

To test lambda functions, use the
`Client.lambda_.invoke <TestLambdaClient.invoke>` method. The
...
example, we'll be using [pytest](https://docs.pytest.org/en/stable/),
...
!!! note
    See the [testing pytest fixtures](testing.md#testing-pytest-fixtures) section for how to use pytest
    fixtures with the Chalice test client.

For testing Lambda functions that are connected to specific events,
you can use the `Client.events` attribute to generate
...
`Client.http` attribute. For example, given this REST API:
...
<a id="testing-boto3-client-calls"></a>
## Testing Boto3 Client Calls

If your event handlers are making AWS API calls using boto3 or botocore,
you can use the [botocore stubber](https://botocore.amazonaws.com/v1/documentation/api/latest/reference/stubber.html)
...
<a id="testing-pytest-fixtures"></a>
## Pytest Fixtures
...
[test fixtures](https://docs.pytest.org/en/stable/fixture.html) to
...
from the [testing lambda functions](testing.md#testing-lambda-functions) section can be rewritten
to use this fixture:
...
[previous section](testing.md#testing-boto3-client-calls) in a more simplified
...
client, see the [test client](../api.md#testing-api) section in the API
documentation.
